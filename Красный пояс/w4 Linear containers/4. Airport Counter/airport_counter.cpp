...
#define ASSERT_EQUAL_ITEM(idx, expected_enum, expected_count) \
  ...
  {
    LOG_DURATION("TestMoscow: GetItem");
...
void TestManyConstructions() {
  ...
  {
    LOG_DURATION("TestManyConstruct: 100'000'000 array of two");
    uint64_t total = 0;
...
void TestManyGetItems() {
  ...
  {
    LOG_DURATION("TestManyGetItems: 100'000'000 get items");
    uint64_t total = 0;
    for (int step = 0; step < 100'000'000; ++step) {
...
void TestMostPopularAirport() {
  ...
  {
    LOG_DURATION("TestMostPopularAirport: most popular");
